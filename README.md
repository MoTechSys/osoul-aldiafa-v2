...
> ⚠️ **نطاق الخدمة المعلَن (ثابت في كل مكان)**: منطقتا **مكة المكرمة** و**المدينة المنورة** — ٥ مدن: جدة · مكة المكرمة · المدينة المنورة · ينبع · بدر.
> هذا النطاق مصدره الوحيد `CITIES` في `src/lib/localPages.ts` ومنه تُشتَق `SERVICE_AREAS` (GeoCircle 30 كم) في `src/lib/schema.ts`.
> **أي نص يقول «جميع مناطق المملكة» أو «كامل المملكة» يخالف البيانات المهيكلة ويجب حذفه** — أُزيلت ١٥ مخالفة من هذا النوع، آخرها في `public/manifest.json`.
