...
if (process.env.CHECK_LINKS === "1") {
  ...
  for (const url of sameAs) {
    try {
      ...
      // snapchat.com يُرجع 404 لأي عميل غير متصفح **حتى للحسابات الموجودة**
      // — أُثبت تجريبيًا (2026-08-29): /add/team.snapchat (الحساب الرسمي
      // لسناب شات نفسها) يُرجع 404 عبر curl تمامًا كحسابنا. إذًا 404 من
      // هذا النطاق حجب مضاد للبوتات متنكّر، لا دليل على رابط ميت.
      const botBlocked404 = res.status === 404 && new URL(url).hostname.endsWith("snapchat.com");
      if (res.status === 403 || res.status === 429 || botBlocked404) {
